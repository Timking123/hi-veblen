...
describe('集成测试 33.1: 完整游戏流程', () => {
  ...
  describe('第一关流程', () => {
    it('应该从第一关开始', () => {
      ...
      expect(stageManager.getCurrentStage().name).toBe('家里')
    })

    it('应该能够生成普通敌人', () => {
      const { type, isElite } = stageManager.spawnEnemy()
      expect([EnemyType.WHITE, EnemyType.GREEN, EnemyType.BLUE]).toContain(type)
    ...
    it('应该在击杀所有敌人后生成 BOSS', () => {
      ...
      expect(bossType).toBe(EnemyType.PURPLE)
    })
  ...
  describe('第二关流程', () => {
    ...
    it('应该进入第二关', () => {
      ...
      expect(stageManager.getCurrentStage().name).toBe('学校')
    })
    ...
    it('应该在击杀所有敌人后生成第二关 BOSS', () => {
      ...
      const bossType = stageManager.spawnBoss()
      expect(bossType).toBe(EnemyType.ORANGE)
    })
  })
  ...
  describe('第三关（最终关）流程', () => {
    ...
    it('应该进入第三关（最终关）', () => {
      ...
      expect(stageManager.getCurrentStage().name).toBe('公司')
      expect(stageManager.isFinalStage()).toBe(true)
    })

    it('应该能够生成第三关的敌人', () => {
      const { type } = stageManager.spawnEnemy()
      expect([EnemyType.PURPLE, EnemyType.YELLOW, EnemyType.ORANGE, EnemyType.RED]).toContain(type)
    })
    ...
    it('应该在击杀所有敌人后生成最终 BOSS', () => {
      ...
      const bossType = stageManager.spawnBoss()
      expect(bossType).toBe(EnemyType.RED)
    })
