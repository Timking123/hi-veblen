...
本页区分已修复的应用质量问题与历史、运维证据。具体提交的通过情况以同一提交的 CI 为准；历史记录不代表当前线上状态。

## 应用质量修复

根项目 lint 与完整 Vitest、Admin backend 完整 Jest 已纳入现有阻断任务。测试修复保留属性样本与真实业务路径，修正旧页面选择器、时间边界、测试资源隔离和 CLI 子进程断言。审计 CLI 的报告写入失败会传播为非零退出码。

游戏实体在入队和画布缩放时接收实际边界；更新或绘制失败会停止循环并显示可重试错误。新的浏览器用例实际执行桌面/触摸移动、缩放和晚帧失败，并在重试后确认新引擎继续响应输入。联系页下载具有失败反馈、超时和卸载取消；留言表单抑制同一轮重复提交；本地留言解码保持原四行格式并正确处理反斜杠与换行。

后端修复包含文件名规范化闭合、SQLite 时间比较、内容标识冲突、测试文件根隔离及同毫秒备份防覆盖/保留。真实 API 运维、生产认证、数据迁移与部署仍是单独边界。

## 2026-07-11 全量诊断

以下计数及生产状态均为历史快照；本节“当前处理”只指当时处理，不代表修复候选或线上最新状态。
...
上述三项保留为历史问题描述。应用修复在独立工作树实施；原有其他工作树的未提交内容不属于此候选。
